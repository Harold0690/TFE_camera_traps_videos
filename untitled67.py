...
start4 = time.time()
print('Start validation top3 somme pourc 3 17<0.35 : ',strftime('%H', gmtime(start4+7200)),'H',strftime('%M', gmtime(start4)),'min',strftime('%S', gmtime(start4)),'sec')
...
with torch.no_grad():
    for ti,(data, target, name) in enumerate(data_loader):
        ...
        pred1, scores, score, pred12, prob12,score17 = find_video_prediction(list_output)
        ...
        correct_tensor = pred.eq(target.data.expand_as(pred))
        ...
        # cmpt1 += 1
            score17_csv.append(score17)
            target_csv.append(dico_inv.get(int(target)))
            pred_csv.append([dico_inv.get(int(i)) for i in pred1])
            names_csv.append(name)
            score_csv.append(scores)
            preds_csv.append(pred12)
            probs_csv.append(prob12)
            score2_csv.append(score)
...
df_results_valid = pd.DataFrame({'video': names_csv,'target':target_csv,'prediction':pred_csv,'scores17':score17_csv,'scores':score_csv,'score2':score2_csv,'predictions':preds_csv,'probabilities':probs_csv})
df_results_valid.to_csv(r"D:\Harold_TFE\results_validation_top3_somme_pourc_3.csv",sep=";",index = False,encoding= 'latin-1')
