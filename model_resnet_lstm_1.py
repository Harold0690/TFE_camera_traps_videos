...
class CNNLSTM(nn.Module):
    def __init__(self, nbr_cl, path_resnet_model):
        super(CNNLSTM, self).__init__()
        
        model_detection = resnet18(pretrained=True)
        ...
        filtered_dict = {k: v for k, v in model_detection_dict.items() if k in resnet_dict}
        resnet_dict.update(filtered_dict)
...
wandb_H = 512
wandb_L = 1024
wandb_resnet_model_name = 'resnet18_6.pt'
wandb_model_name = 'resnet18_LSTM_1.pt'
wandb_seed = 100
...
model = CNNLSTM(nbr_cl = len(dico), path_resnet_model = path_resnet_model)
print(model)
...
# path_error = r'D:\Harold\model_video\resnet_lstm_error_test'
# if os.path.exists(path_error):
#     shutil.rmtree(path_error)
#     os.makedirs(path_error)
# else:
