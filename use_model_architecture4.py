...
def find_video_prediction(list_output):
    ...
    if (final_pred == 1) and (moy_p_1 < 0.75):
        final_pred = 0
        
        p1 = nbr1 / len(list_output)
             
    return final_pred, moy_p_1, preds, probs, p1
...
model_path = r'C:\resnet18_binaire_1.pt'
path_in = r"C:\video"
...
dataset = CSVVideoDataset(path_in,2,trans)
...
with torch.no_grad():
    for ti,(data, target, name) in enumerate(data_loader):
        ...
        final_pred, moy_p_1, preds, probs, p1 = find_video_prediction(list_output)
...
df_results_valid = pd.DataFrame({'video': names_csv,'prediction_top1':y_pred})
df_results_valid.to_csv(path_csv_out,sep=";",index = False,encoding= 'latin-1')
