...
wandb_H = 225
wandb_L = 450
wandb_model_name = 'resnet50_video_1.pt'
wandb_seed = 100
...
path_csv_train = r'D:\Harold\model_video\video_train20.csv'
path_csv_test = r'D:\Harold\model_video\video_test20.csv'
path_csv_valid = r'D:\Harold\model_video\video_valid20.csv'
...
with torch.no_grad():
    for ti,(data, target, name) in enumerate(valid_loader):
        ...
        y_pred1.append(int(pred_top3[0][0]))
        y_pred2.append(int(pred_top3[0][1]))
        y_pred3.append(int(pred_top3[0][2]))
        label = target.data
...
df_results_valid = pd.DataFrame({'images': names10,'target':y_true,'labels':y_pred,'labels1':y_pred1,'labels2':y_pred2,'labels3':y_pred3})
df_results_valid.to_csv(r"\\gf009pc059\Datafast\Harold\model_video\results_validation_resnet_video_1.csv",sep=";",index = False,encoding= 'latin-1')
...
df_results_valid = pd.DataFrame({'images': names10,'target':y_true,'labels':y_pred,'labels1':y_pred1,'labels2':y_pred2,'labels3':y_pred3})
df_results_valid.to_csv(r"\\gf009pc059\Datafast\Harold\model_video\results_test_resnet_video_1.csv",sep=";",index = False,encoding= 'latin-1')
